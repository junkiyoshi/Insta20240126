...
void ofApp::update() {
	...
	float noise_threshold = 0.52;
	
	ofColor line_color = ofColor(255);

	for(float radius = 230; radius > 180; radius -= 5){

		ofColor face_color = ofColor(ofMap(radius, 230, 180, 39, 239));
		...
		for (float phi_deg = 0; phi_deg < 360; phi_deg += phi_deg_step) {
			...
			for (float theta_deg = 0; theta_deg <= 180; theta_deg += theta_deg_step) {
				...
				auto noise_value = ofNoise(
					cos(phi_deg * DEG_TO_RAD),
					sin(phi_deg * DEG_TO_RAD),
				...
				auto noise_value_1 = ofNoise(
					cos(phi_deg * DEG_TO_RAD),
					sin(phi_deg * DEG_TO_RAD),
					cos((theta_deg - theta_deg_step) * DEG_TO_RAD),
					radius * 0.01 + ofGetFrameNum() * 0.04);
				auto noise_value_2 = ofNoise(
					cos((phi_deg + phi_deg_step) * DEG_TO_RAD),
					sin((phi_deg + phi_deg_step) * DEG_TO_RAD),
					cos(theta_deg * DEG_TO_RAD),
					radius * 0.01 + ofGetFrameNum() * 0.04);
